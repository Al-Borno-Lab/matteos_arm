...
class ReachEnvV0(BaseV0):
    ...
    def _setup(self,
        ...
        self.near_th = 0.03 #TODO: Change this
    ...
    def get_reward_dict(self, obs_dict): # set up reward dictionary
        ...
        biarticular_act_mag = abs(np.linalg.norm(self.obs_dict['act'][..., 4:6], axis=-1) / self.sim.model.na if self.sim.model.na != 0 else 0)
        ...
        rwd_dict = collections.OrderedDict((
            ...
            ('act_reg', (-1/6) * act_mag),
            ('penalty',
                -0.0008 * (self.time * 100) ** 1.5
                + -1.0 * (reach_dist > far_th)
                + (-0.5 * np.abs(palm[0][0][1] - self.obj_xyz_range[0][1]))
                - 0.1 * biarticular_act_mag
            ),
            ('sparse', -1. * reach_dist),
        ...
        rwd_dict['dense'] = float(np.sum([wt * float(rwd_dict[key]) for key, wt in self.rwd_keys_wt.items()]))
